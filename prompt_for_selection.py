...
def generate_selection(context):
  messages = f'''
          당신의 역할은 선택지들 중에서 **배려/공감이 가장 적은** 선택지를 선택하고, 이것이 내가 행동해야 하는 최적의 선택임에 타당한 논리를 제공하는 것입니다.
          인풋은 상황과 선택지로 총 5개로 구성되며, 첫번째 인덱스를 제외한 나머지 4개의 선택지 중에서 하나를 골라 해당 선택지의 번호와 줄글로 된 그 이유를 생성하면 됩니다.
          
          ** 주의 사항 **
          최종 출력 포맷은 리스트로 출력할 것 ex) [선택지 번호(정수형), '선택 이유']
          대상은 중/고등학생으로 선택 이유에 알아듣기 쉬운 단어를 사용하고 나쁜 선택을 해도 **뻔뻔하게** 자신의 주장을 굽히지 말고 행동할 것.
          자신이 선택한 선택지를 다시한번 제대로 읽고, 그러한 선택을 한 이유에 대해 논리적으로 타당한 이유를 제시할 것.
          ...
          ** 예시 **
          ...
          ('그냥 아무 말 안 하고 내가 더 하고 싶은 걸 선택한다.'를 선택)
          답안:
...
if __name__=="__main__":
  result = generate_selection('지우개 훔치기')
  print(result)